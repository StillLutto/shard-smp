...
class TitansEdgeStrengthRunnable(private val shardSMP: ShardSMP) : BukkitRunnable() {
    ...
    override fun run() {
        for (onlinePlayer in Bukkit.getOnlinePlayers()) {
            for (item in onlinePlayer.inventory) {
                ...
                if (item.itemMeta.persistentDataContainer[customItemKey, PersistentDataType.STRING] != "titans_edge") continue
                ...
                onlinePlayer.addPotionEffect(PotionEffect(PotionEffectType.INCREASE_DAMAGE, 21, 1))
            }
        }
    }
