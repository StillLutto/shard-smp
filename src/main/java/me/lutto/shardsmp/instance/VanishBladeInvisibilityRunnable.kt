...
class VanishBladeInvisibilityRunnable(private val shardSMP: ShardSMP) : BukkitRunnable() {
    ...
    override fun run() {
        for (onlinePlayer in Bukkit.getOnlinePlayers()) {
            for (item in onlinePlayer.inventory) {
                ...
                onlinePlayer.addPotionEffect(PotionEffect(PotionEffectType.INVISIBILITY, 21, 0))
            }
        }
    }
