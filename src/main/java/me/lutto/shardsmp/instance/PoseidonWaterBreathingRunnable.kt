...
class PoseidonWaterBreathingRunnable(private val shardSMP: ShardSMP) : BukkitRunnable() {
    ...
    override fun run() {
        for (onlinePlayer in Bukkit.getOnlinePlayers()) {
            for (item in onlinePlayer.inventory) {
                ...
                if (item.itemMeta.persistentDataContainer[customItemKey, PersistentDataType.STRING] != "poseidon_trident") continue
                ...
                onlinePlayer.addPotionEffect(PotionEffect(PotionEffectType.WATER_BREATHING, 21, 0))
            }
        }
    }
